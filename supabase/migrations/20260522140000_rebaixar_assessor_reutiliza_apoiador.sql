-- Casos reais de legado/consistência: o mesmo utilizador ficou com linha activa em apoiadores
-- (profile_id) enquanto ainda era equipa técnica (assessor). Rebaixar não pode falhar sempre
-- com «Já existe um cadastro de apoiador ativo»: reaproveitar e actualizar esse registro.
...
COMMENT ON FUNCTION public.rebaixar_assessor_para_papel(uuid, text) IS
  ...
  'reaproveita apoiador já activo mesmo profile se existir; reparentiza apoiadores/votantes filhos antes do DELETE.';
