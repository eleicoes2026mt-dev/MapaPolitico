-- Resolve v_camp pelo assessor técnico do deputado (`assessores` onde profile_id = raiz da campanha).
-- Fallback para app_assessor_id_do_candidato(). Evita v_camp NULL / incorreto se o JWT ou metadados
-- estiverem fora do padrão, mantendo FK reparentização e DELETE coerentes.
...
COMMENT ON FUNCTION public.rebaixar_assessor_para_papel(uuid, text) IS
  'Gestor da campanha: rebaixa assessor para apoiador ou votante «Amigos do Gilberto»; v_camp = assessor onde profile_id = raiz da campanha (fallback app_assessor_id_do_candidato); '
  'reaproveita apoiador ativo mesmo profile se existir; reparentiza apoiadores/votantes filhos antes do DELETE.';
