-- Rebaixa assessor para apoiador ou para cadastro «Amigos do Gilberto» (votante).
-- Gestor da campanha (app_is_candidato); remove apenas a linha em assessores; mantém UUID do perfil.
...
COMMENT ON FUNCTION public.rebaixar_assessor_para_papel(uuid, text) IS
  'Gestor da campanha: rebaixa assessor para apoiador ou votante «Amigos do Gilberto», preservando profile_id para rede/indicações.';

GRANT EXECUTE ON FUNCTION public.rebaixar_assessor_para_papel(uuid, text) TO authenticated;
