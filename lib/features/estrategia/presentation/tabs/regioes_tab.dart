...
class _RegioesTabState extends ConsumerState<RegioesTab> {
              ...
              if (fundidas.isEmpty) {
                return Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    ...efetivas.map(
                        (r) => _buildRegiaoCard(context, theme, r, isAdmin)),
                  ],
                );
              }
              return Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: efetivas
                    .map((r) => _buildRegiaoCard(context, theme, r, isAdmin))
                    .toList(),
              );
            },
  ...
  Widget _buildRegiaoCard(
      BuildContext context, ThemeData theme, RegiaoEfetiva r, bool isAdmin) {
    return Card(
              ...
              ? () async {
                  ...
                  if (ok && context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text('Nome da região atualizado.')));
                  }
                }
              : null,
  ...
  Future<void> _showFundirDialog(BuildContext context) async {
            ...
            return AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    const Text(
                        'Selecione as regiões que formarão uma nova região com um único nome:'),
    ...
    if (saved == true && nomeController.text.trim().isNotEmpty) {
      ...
      final fundida =
          RegiaoFundida(id: id, nome: nomeController.text.trim(), ids: ids);
  ...
  Future<void> _desfazer(BuildContext context) async {
    ...
    if (context.mounted && ok) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Última ação desfeita.')));
    }
  }
  ...
  Future<void> _restaurarNomesPadrao(BuildContext context) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Nomes restaurados para o padrão do mapa.')),
      );
    }
  }
  ...
  Future<void> _removerFusao(BuildContext context, String id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'As regiões voltarão a aparecer separadamente. Deseja continuar?'),
        actions: [
          TextButton(
              onPressed: () => Navigator.of(ctx).pop(false),
              child: const Text('Cancelar')),
    ...
    if (confirm == true) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Fusão removida.')));
      }
    }
  }
}
