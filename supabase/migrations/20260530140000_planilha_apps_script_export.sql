-- Exportação para Google Apps Script (sem chave JSON da conta de serviço).
-- Use quando a organização bloqueia iam.disableServiceAccountKeyCreation.

ALTER TABLE public.planilha_sync_config
  ADD COLUMN IF NOT EXISTS apps_script_webapp_url text;

COMMENT ON COLUMN public.planilha_sync_config.apps_script_webapp_url IS
  'URL do Web App Apps Script (Implantar → App da Web). Se preenchida, triggers usam esta URL em vez da Edge Function.';
